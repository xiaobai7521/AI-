      ...
      <div class="chat-messages">
        ...
        >
          ...
          <div class="message-content">
            ...
            <div v-if="msg.fromSessionList" class="message-time">
              {{
...
const sendMessage = () => {
  ...
  if (isAiTyping.value) {
    ElMessage('AI助手正在输入中，请稍后')
    return
  }
  const message = userMessage.value.trim()
  userMessage.value = ''

  // 如果没有会话或者是临时会话需要创建一个新的绘画记录
...
const handleSessionClick = session => {
  ...
  getSessionDetail(session.id).then(res => {
    console.log(res)
    // 为从会话列表加载的消息添加标记
    const messagesWithSessionMark = res.map(msg => ({
      ...msg,
      fromSessionList: true
    }))
    messages.value = messagesWithSessionMark
  })
  // 更新当前会话对象数据
  const sessionData = {
