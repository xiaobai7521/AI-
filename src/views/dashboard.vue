  ...
  <div class="dashboard-container">
    ...
    <el-row style="margin-top: 20px" :gutter="20">
      ...
      <el-col :span="12">
        <el-card style="width: 100%">
          <template #header>
            <div class="card-header">咨询绘画统计</div>
          </template>
          <div class="chart-content">
...
const initEmotionChart = () => {
  ...
  // 获取数据
  const TrendDate = aiData.value.emotionTrend
  // 配置项
  const option = {
    title: {
      text: '情绪趋势分析',
    ...
    xAxis: {
      type: 'category',
      data: TrendDate.map(item => item.date),
      axisLine: {
        lineStyle: {
          color: '#2d3436'
        }
    ...
    series: [
      {
        name: '平均情绪评分',
        type: 'line',
        data: TrendDate.map(item => item.avgMoodScore),
        smooth: true,
        lineStyle: {
          width: 3,
          color: '#faebaf'
      ...
      },
      {
        name: '记录数量',
        type: 'line',
        data: TrendDate.map(item => item.recordCount),
        lineStyle: {
          color: '#eebsa3'
        },
        itemStyle: {
